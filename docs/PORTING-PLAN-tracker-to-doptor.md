...
_(Rejected: per-org counter — loses per-team prefixes; a new Projects entity — unwanted surface.)_
...
**DECIDED:** migrate to the tracker's enum _values_ as Postgres enums derived from Doptor's
...
- [ ] Re-audit Doptor's _current_ tasks + attendance modules (docs are stale — H-8 already
...
- File-attachment _storage_ backend if Doptor's `documents`/`files` upload (backlog H-7)
