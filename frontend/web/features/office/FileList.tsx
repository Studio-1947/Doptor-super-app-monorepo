...
export default function FileList({ files, onFileClick }: FileListProps) {
    if (files.length === 0) {
        return (
            <div className="flex flex-col items-center justify-center py-20 bg-white dark:bg-slate-900 border border-dashed border-slate-300 rounded-sm">
                <div className="p-4 bg-slate-50 dark:bg-slate-900/50 rounded-full mb-4">
                    <FileText size={32} className="text-slate-300" />
                </div>
                <h3 className="text-sm font-medium text-slate-900 dark:text-slate-100">No files found</h3>
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">There are no files in this view</p>
            </div>
        );
    }
                        ...
                        <div className="flex items-start justify-between gap-4 pl-2">
                            <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-3 mb-1">
                                    <span className="font-mono text-xs font-medium text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded-sm">
                                        {file.file_number}
                                    </span>
                                ...
                                {file.description && (
                                    <p className="text-sm text-slate-500 dark:text-slate-400 line-clamp-1 mb-3 font-light">
                                        {file.description}
                                    </p>
                                )}
